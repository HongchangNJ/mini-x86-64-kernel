...
[SECTION .gdt]
SEG_BASE equ 0 ;描述符中的基地址
SEG_LIMIT equ 0xfffff; 描述符中的limit，单位由G标志为决定

CODE_SELECTOR equ 1 << 3 ; 段选择子index为1,低三位为标志位。
DATA_SELECTOR equ 2 << 3 ; 段选择子index为2,低三位为标志位。
...
gdt_ptr:
    ; 保存gdt的大小和基址，供lgdt加载。保护模式下共 6个字节48位
    dw $ - gdt_base
    dd gdt_base
